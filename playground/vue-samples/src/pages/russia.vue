...
const options = reactive<Omit<MapOptions, 'selector'>>({
  ...
  visualizeData: {
    ...
    values: {
      'RU-MOW': 100, // Moscow
      'RU-SPE': 85, // Saint Petersburg
      'RU-NIZ': 80, // Nizhny Novgorod
      'RU-SAM': 75, // Samara
      'RU-ROS': 70, // Rostov
      'RU-KGD': 65, // Kaliningrad
    },
  },
  markers: [
    {
...
function addRandomMarker() {
  // Russia boundaries: approximately 41°N to 82°N and 19°E to 190°E
  const lat = 41 + Math.random() * 41 // 41°N to 82°N
  const lng = 19 + Math.random() * 171 // 19°E to 190°E
  const marker = {
    name: `Russian Marker ${(options.markers?.length ?? 0) + 1}`,
...
<template>
  <div class="container">
    <h1>ts-maps Vue Example - Russia Map</h1>
